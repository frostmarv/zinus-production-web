...
// ✅ GUNAKAN cuttingAPI untuk endpoint /api/cutting
import { cuttingAPI } from "../../api/cutting";
import "../../styles/Cutting/BalokCutting.css";

// ✅ Nama komponen diubah menjadi BalokCutting
const BalokCutting = () => {
  // Header Info
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ GUNAKAN HELPER API cuttingAPI (akan memanggil POST /api/cutting)
      await cuttingAPI.save(dataToSend);

      console.log("✅ Data berhasil disimpan ke /api/cutting:", dataToSend);
      ...
      console.error("🚨 Error koneksi ke backend /api/cutting:", error);
      alert(
        `❌ Error koneksi ke backend (/api/cutting).\n` +
          `Pastikan:\n` +
          `1. Backend jalan\n` +
          `2. URL benar di .env\n` +
  ...
  return (
    <div className="cutting-container">
      <div className="cutting-header">
        <h1>📦 Balok Cutting Management</h1> {/* Judul diubah */}
        <p>Advanced balok cutting data input system</p> {/* Deskripsi diubah */}
      </div>
          ...
          {actualData.map((actual, index) => (
            <div key={actual.id} className="balok-card">
              ...
              <div className="form-grid">
                ...
                {/* Total dan Cutting Loss di luar grid utama atau di bagian bawah */}
